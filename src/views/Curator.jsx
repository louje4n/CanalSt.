...
export const Curator = ({
  filtered, query, setQuery, lin, setLin, era, setEra, sizeF, setSizeF, priceMax, setPriceMax, openPdv, toggle
}) => (
  <div>
    <div style={{ padding:"20px 20px 0", borderBottom:"1px solid #f0f0f0", position:"sticky", top:0, background:"#fff", zIndex:10 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, borderBottom:"2px solid #111", paddingBottom:10, marginBottom:18 }}>
        ...
        <input placeholder="Brand, lineage, reference…" value={query} onChange={e => setQuery(e.target.value)} style={{ flex:1 }}/>
        {query && <button onClick={() => setQuery("")} style={{ background:"none", border:"none", cursor:"pointer", ...M, fontSize:10, color:"#bbb" }}>✕</button>}
      </div>
      <div style={{ paddingBottom:14 }}>
        ...
        <div className="hscroll" style={{ paddingBottom:10, marginBottom:12 }}>
          {LINS.map(v => <button key={v} className={`chip${lin.includes(v)?" on":""}`} onClick={() => toggle(lin, setLin, v)}>{v}</button>)}
        </div>
        <div style={{ marginBottom:12 }}>
          <div style={{ ...M, fontSize:8, letterSpacing:".12em", color:"#aaa", marginBottom:9 }}>ERA</div>
          <div className="hscroll">
            {ERAS.map(v => <button key={v} className={`chip${era.includes(v)?" on":""}`} onClick={() => toggle(era, setEra, v)}>{v}</button>)}
          </div>
        </div>
        <div style={{ marginBottom:12 }}>
          <div style={{ ...M, fontSize:8, letterSpacing:".12em", color:"#aaa", marginBottom:9 }}>SIZE</div>
          <div className="hscroll">
            {SIZES.map(v => <button key={v} className={`chip${sizeF.includes(v)?" on":""}`} onClick={() => toggle(sizeF, setSizeF, v)}>{v}</button>)}
          </div>
        </div>
        <div style={{ marginBottom:14 }}>
          ...
          <input type="range" min="100" max="5000" step="100" value={priceMax} onChange={e => setPriceMax(+e.target.value)} style={{ width:"100%", accentColor:"#111" }}/>
        </div>
      </div>
    </div>
    ...
    {filtered.map(l => (
      <div key={l.id} onClick={() => openPdv(l)} className="row-hover"
